...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BlendSpace)
...
void UBlendSpace::ResetBlendSamples(TArray<FBlendSampleData>& InOutSampleDataCache, float InNormalizedCurrentTime, bool bLooping, bool bMatchSyncPhases) const
{
	...
	else
	{
		...
		for (int32 SampleIndex = 0; SampleIndex < InOutSampleDataCache.Num(); ++SampleIndex)
		{
			...
			SampleDataItem.MarkerTickRecord.Reset();
			SampleDataItem.PreviousTime = InNormalizedCurrentTime * SampleData[SampleDataItem.SampleDataIndex].GetSamplePlayLength();
			SampleDataItem.Time = InNormalizedCurrentTime * SampleData[SampleDataItem.SampleDataIndex].GetSamplePlayLength();
		}
	}
}
			...
			if (bCanDoMarkerSync)
			{
				...
				else
				{
					...
					if (LeaderSample.Animation)
					{
						...
						{
							LeaderSampleData.MarkerTickRecord.Reset();
							LeaderSampleData.Time = NormalizedCurrentTime * LeaderSample.GetSamplePlayLength();
						...
						NormalizedCurrentTime = LeaderSampleData.Time / LeaderSample.GetSamplePlayLength();
						*Instance.MarkerTickRecord = LeaderSampleData.MarkerTickRecord;
					}
				}
			}
			...
			if (bCanDoMarkerSync)
			{
				...
				if (Sample.Animation)
				{
					...
					if (Context.GetDeltaTime() != 0.f)
					{
						if (!Instance.MarkerTickRecord->IsValid(Instance.bLooping))
						{
							SampleDataItem.Time = NormalizedCurrentTime * Sample.GetSamplePlayLength();
						}
					...
					*Instance.MarkerTickRecord = SampleDataItem.MarkerTickRecord;
					NormalizedCurrentTime = SampleDataItem.Time / Sample.GetSamplePlayLength();
				}
			}
			else
			{
				...
				if (SampleData.IsValidIndex(SampleDataIndex) && (SampleEntry.TotalWeight > ZERO_ANIMWEIGHT_THRESH))
				{
					...
					if (Sample.Animation)
					{
						...
						if (!bCanDoMarkerSync || Sample.Animation->AuthoredSyncMarkers.Num() == 0) //Have already updated time if we are doing marker sync
						{
							...
							{
								PrevSampleDataTime = CurrentSampleDataTime = Sample.Animation->GetTimeAtFrame(Sample.FrameIndexToSample);
							}
							else
							{
								PrevSampleDataTime = SampleNormalizedPreviousTime * Sample.GetSamplePlayLength();
								CurrentSampleDataTime = SampleNormalizedCurrentTime * Sample.GetSamplePlayLength();
							}
						}
						else
						{
...
void UBlendSpace::ReplaceReferredAnimations(const TMap<UAnimationAsset*, UAnimationAsset*>& ReplacementMap)
{
	...
	for (FBlendSample& Sample : SampleData)
	{
		...
		if (Sample.Animation)
		{
			...
			{
				if (UAnimSequence* ReplacementAnimSequence = Cast<UAnimSequence>(*ReplacementAsset))
				{
					Sample.Animation = ReplacementAnimSequence;
	...
	if (PreviewBasePose)
	{
		if (UAnimationAsset* const* ReplacementAsset = ReplacementMap.Find(PreviewBasePose))
		{
			if (UAnimSequence* ReplacementAnimSequence = Cast<UAnimSequence>(*ReplacementAsset))
			{
				PreviewBasePose = ReplacementAnimSequence;
...
/** [翻译失败: When per-bone blend data are required to be sorted, this stores the sorted copy and the index of the original] */
struct FSortedPerBoneInterpolation
{
	...
	/** [翻译失败: Index into the original array] */
	int32 OriginalIndex;
};
...
void UBlendSpace::ValidateSampleData()
{
	...
	for (int32 SampleIndex = 0; SampleIndex < SampleData.Num(); ++SampleIndex)
	{
		...
		if (IsAsset())
		{
			const bool bAnimationExists = Sample.Animation != nullptr;
			const bool bSampleInBounds = bAnimationExists ? IsSampleWithinBounds(Sample.SampleValue) : true;
			const bool bSampleIsUnique = bAnimationExists ? !IsTooCloseToExistingSamplePoint(Sample.SampleValue, SampleIndex) : true;
			const bool bShouldForceConditionalPostLoad = bAnimationExists && FUObjectThreadContext::Get().IsRoutingPostLoad && !Sample.Animation->IsDataModelValid();
			if (bShouldForceConditionalPostLoad)
			{
				Sample.Animation->ConditionalPostLoad();
			}
			const bool bSingleFrameBlendingIndexInBounds = IsSingleFrameBlendingIndexInBounds(Sample);
			Sample.bIsValid = bAnimationExists && bSampleInBounds && bSampleIsUnique && bSingleFrameBlendingIndexInBounds;
			...
			else
			{
				...
				else
				{
					...
					else if (!bSingleFrameBlendingIndexInBounds)
					{
						Message->AddToken(FTextToken::Create(HasSampleText));
						Message->AddToken(FAssetNameToken::Create(Sample.Animation->GetPathName(), FText::FromString(Sample.Animation->GetName())));
						Message->AddToken(FTextToken::Create(LOCTEXT("EmptyAnimationData_SingleFrameBlendingNotInBounds", "single frame blending index is out of bounds. Recommend adjusting it.")));
						
						UE_LOG(LogAnimation, Warning, TEXT("[%s : %d] - single frame blending index: [%d] is out of bounds"), *GetFullName(), SampleIndex + 1, Sample.FrameIndexToSample);
					} 
					else
					{
						// Shouldn't get here
...
bool UBlendSpace::IsSingleFrameBlendingIndexInBounds(const FBlendSample& BlendSample)
{
	if (BlendSample.bUseSingleFrameForBlending)
	{
		if (BlendSample.Animation && BlendSample.Animation->IsDataModelValid())
		{
			const uint32 NumberOfFrames = static_cast<uint32>(BlendSample.Animation->GetDataModelInterface()->GetNumberOfFrames());		
			if (BlendSample.FrameIndexToSample >= 0 && BlendSample.FrameIndexToSample <= NumberOfFrames)
			{
				return true;
			}
		}
...
bool UBlendSpace::ValidateSampleValue(const FVector& SampleValue, int32 OriginalIndex) const
{
	...
	bValid &= OriginalIndex == INDEX_NONE || IsSingleFrameBlendingIndexInBounds(SampleData[OriginalIndex]);
	return bValid;
}
...
float FBlendSample::GetSamplePlayLength() const
{
	if (Animation)
	{
		if (bUseSingleFrameForBlending)
		{
			return Animation->GetSamplingFrameRate().AsInterval();
		}
