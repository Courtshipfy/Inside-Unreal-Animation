...
bool UAnimCurveMetaData::GetCurveMetaDataMaterial(FName CurveName) const
{
	const FCurveMetaData* MetaData = GetCurveMetaData(CurveName);
	if (MetaData)
	{
		return MetaData->Type.bMaterial;
	}

	return false;
}

bool UAnimCurveMetaData::GetCurveMetaDataMorphTarget(FName CurveName) const
{
	const FCurveMetaData* MetaData = GetCurveMetaData(CurveName);
	if (MetaData)
	{
		return MetaData->Type.bMorphtarget;
	}
