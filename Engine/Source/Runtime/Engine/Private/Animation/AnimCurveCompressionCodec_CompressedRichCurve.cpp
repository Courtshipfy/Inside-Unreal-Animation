...
void UAnimCurveCompressionCodec_CompressedRichCurve::DecompressCurves(const FCompressedAnimSequence& AnimSeq, FBlendedCurve& Curves, float CurrentTime) const
{
	...
	auto GetValueFromIndex = [&IndexedCurveNames, Buffer, &Args](int32 InCurveIndex)
	{
		...
		Args.CompressionFormat = Curve.CompressionFormat;
		Args.KeyTimeCompressionFormat = Curve.KeyTimeCompressionFormat;
		Args.PreInfinityExtrap = Curve.PreInfinityExtrap;
		Args.PostInfinityExtrap = Curve.PostInfinityExtrap;
		Args.ConstantValueNumKeys = Curve.ConstantValueNumKeys;
		Args.CompressedKeys = CompressedKeys;

		const float Value = FCompressedRichCurve::StaticEval(Args);
		return Value;
	};
