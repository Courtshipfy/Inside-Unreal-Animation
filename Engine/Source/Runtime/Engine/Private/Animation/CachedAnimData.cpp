...
float FCachedAnimTransitionData::GetTimeElapsed(UAnimInstance& InAnimInstance) const
{
	CacheIndices(InAnimInstance);

	if (MachineIndex != INDEX_NONE && TransitionIndex != INDEX_NONE)
	{
		return InAnimInstance.GetInstanceTransitionTimeElapsed(MachineIndex, TransitionIndex);
	}

	return 0.f;
}

float FCachedAnimTransitionData::GetTimeElapsedFraction(UAnimInstance& InAnimInstance) const
{
	CacheIndices(InAnimInstance);

	if (MachineIndex != INDEX_NONE && TransitionIndex != INDEX_NONE)
	{
		return InAnimInstance.GetInstanceTransitionTimeElapsedFraction(MachineIndex, TransitionIndex);
	}
