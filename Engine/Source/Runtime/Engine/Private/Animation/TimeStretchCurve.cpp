 ...
 // 不要修剪最后一个标记，我们需要它来描述动画的结束。
	{
		...
		for (int32 SegmentIndex = 0; SegmentIndex < NumSegments - 2; SegmentIndex++)
		{
			...
			{
				Markers.RemoveAt(MarkerIndex + 1, EAllowShrinking::No);
			}
			else
			{
				MarkerIndex++;
	...
	*/
	{
		for (int32 MarkerIndex = 0; MarkerIndex < P_Marker_Original.Num() - 1; MarkerIndex++)
		{
			...
			{
				P_Marker_Target.RemoveAt(MarkerIndex + 1, EAllowShrinking::No);
				P_Marker_Original.RemoveAt(MarkerIndex + 1, EAllowShrinking::No);
				MarkerIndex--;
			}
		}
