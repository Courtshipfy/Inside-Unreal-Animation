...
{
	const int32 RawBoneIndex = FindRawBoneIndex(InBoneName);
	const bool bCaseSensitiveOnly = InBoneName == InNewName && !InBoneName.IsEqual(InNewName, ENameCase::CaseSensitive);
	const int32 RawNewBoneIndex = bCaseSensitiveOnly ? INDEX_NONE : FindRawBoneIndex(InNewName);
...
namespace
{
void ComposeAbsoluteTransforms(
	const FReferenceSkeleton& InRefSkeleton,
	TConstArrayView<FTransform> InBoneSpaceTransforms,
	TArray<FTransform>& OutAbsoluteTransforms
	)
{
	OutAbsoluteTransforms.SetNumUninitialized(InBoneSpaceTransforms.Num());

	for (int32 BoneIndex = 0; BoneIndex < InBoneSpaceTransforms.Num(); BoneIndex++)
	{
		const int32 ParentIndex = InRefSkeleton.GetParentIndex(BoneIndex);
		if (ParentIndex >= 0)
		{
			OutAbsoluteTransforms[BoneIndex] = InBoneSpaceTransforms[BoneIndex] * OutAbsoluteTransforms[ParentIndex];
		}
		else
		{
			OutAbsoluteTransforms[BoneIndex] = InBoneSpaceTransforms[BoneIndex];
		}
	}
}

FTransform ComposeSingleAbsoluteTransform(
	const int32 InBoneIndex,
	const FReferenceSkeleton& InRefSkeleton,
	TConstArrayView<FTransform> InBoneSpaceTransforms
	)
{
	FTransform AbsolutePose = InBoneSpaceTransforms[InBoneIndex];

	int32 ParentIndex = InRefSkeleton.GetParentIndex(InBoneIndex);

	while(ParentIndex != INDEX_NONE)
	{
		AbsolutePose = AbsolutePose * InBoneSpaceTransforms[ParentIndex];
		ParentIndex = InRefSkeleton.GetParentIndex(ParentIndex);
	}
...
{
	ComposeAbsoluteTransforms(*this, RawRefBonePose, OutAbsoluteTransforms);
}

void FReferenceSkeleton::GetBoneAbsoluteTransforms(TArray<FTransform>& OutAbsoluteTransforms) const
{
	ComposeAbsoluteTransforms(*this, FinalRefBonePose, OutAbsoluteTransforms);
}

FTransform FReferenceSkeleton::GetRawBoneAbsoluteTransform(const int32 InRawBoneIndex) const
{
	if (ensure(IsValidRawIndex(InRawBoneIndex)))
	{
		return ComposeSingleAbsoluteTransform(InRawBoneIndex, *this, RawRefBonePose);
	}
...
{
	if (ensure(IsValidIndex(InBoneIndex)))
	{
		return ComposeSingleAbsoluteTransform(InBoneIndex, *this, FinalRefBonePose);
	}
...
void FReferenceSkeleton::EnsureParentsExist(TArray<FBoneIndexType>& InOutBoneSortedArray) const
{
 ...
 // 迭代现有数组。
  ...
  // 对于根骨骼，继续前进。
   ...
   // 如果数组中没有这个父级，我们将它添加到这个位置，并将“i”保留在原来的位置。
