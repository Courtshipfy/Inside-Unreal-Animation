...
void Attributes::OverrideAttributes(const FStackAttributeContainer& SourceAttributes, FStackAttributeContainer& OutAttributes, float Weight)
{
	...
	const FStackAttributeContainer* Array[] = { &SourceAttributes };
	for (const TWeakObjectPtr<UScriptStruct>& WeakScriptStruct : UniqueStructs)
	{
		const IAttributeBlendOperator* Operator = AttributeTypes::GetTypeOperator(WeakScriptStruct);
...
void Attributes::ConvertToAdditive(const FStackAttributeContainer& BaseAttributes, FStackAttributeContainer& OutAdditiveAttributes)
{
	...
	const FStackAttributeContainer* Array[] = { &BaseAttributes };
	for (const TWeakObjectPtr<UScriptStruct>& WeakScriptStruct : UniqueStructs)
	{
		const IAttributeBlendOperator* Operator = AttributeTypes::GetTypeOperator(WeakScriptStruct);
...
void Attributes::BlendAttributesPerBone(TArrayView<const FStackAttributeContainer> SourceAttributes, TArrayView<const int32> PerBoneInterpolationIndices, TArrayView<const FBlendSampleData> BlendSampleDataCache, FStackAttributeContainer& OutAttributes)
{
 ...
 // 如果没有每个骨骼的权重，则可以将属性与容器级权重混合
	...
	{
		TArrayView<float> Weights((float*)FMemory_Alloca(BlendSampleDataCache.Num() * sizeof(float)), BlendSampleDataCache.Num());
		for (int32 WeightIndex = 0; WeightIndex < BlendSampleDataCache.Num(); ++WeightIndex)
		{
			Weights[WeightIndex] = BlendSampleDataCache[WeightIndex].GetClampedWeight();
		}
		...
		for (const TWeakObjectPtr<UScriptStruct>& WeakScriptStruct : UniqueStructs)
		{
			...
			FAttributeBlendData AttributeBlendData = FAttributeBlendData::PerContainerWeighted(SourceAttributes, Weights, WeakScriptStruct.Get());
			Operator->Blend(AttributeBlendData, &OutAttributes);
		}
	}
}
...
void Attributes::BlendAttributesPerBone(TArrayView<const FStackAttributeContainer> SourceAttributes, TArrayView<const int32> PerBoneInterpolationIndices, TArrayView<const FBlendSampleData> BlendSampleDataCache, TArrayView<const int32> BlendSampleDataCacheIndices, FStackAttributeContainer& OutAttributes)
{
 ...
 // 如果没有每个骨骼的权重，则可以将属性与容器级权重混合
	...
	{
		TArrayView<float> Weights((float*)FMemory_Alloca(BlendSampleDataCacheIndices.Num() * sizeof(float)), BlendSampleDataCacheIndices.Num());
		for (int32 WeightIndex = 0; WeightIndex < BlendSampleDataCacheIndices.Num(); ++WeightIndex)
		{
			Weights[WeightIndex] = BlendSampleDataCache[BlendSampleDataCacheIndices[WeightIndex]].GetClampedWeight();
		}
...
void Attributes::BlendAttributesPerBoneFilter(const FStackAttributeContainer& BaseAttributes, const TArrayView<const FStackAttributeContainer> BlendAttributes, const TConstArrayView<FPerBoneBlendWeight> BoneBlendWeights, FStackAttributeContainer& OutAttributes)
{
...
void Attributes::MirrorAttributes(FStackAttributeContainer& CustomAttributes, const UMirrorDataTable& MirrorTable, const TArray<FCompactPoseBoneIndex>& CompactPoseMirrorBones)
{
 ...
 // @todo：使用适当大小的 TInlineAllocator 初始化此数组
	...
	for (const TWeakObjectPtr<UScriptStruct>& WeakScriptStruct : CustomAttributes.GetUniqueTypes())
	{
		...
		{
			MirrorSets.Reset();
			const TConstArrayView<int32> UniqueBoneIndices = CustomAttributes.GetUniqueTypedBoneIndices(TypeIndex);
			SortedBoneIndices = UniqueBoneIndices; 
    ...
    // 已弃用的行为支持，删除其他签名时删除
        ...
        // 只处理简单的交换 - 两个条目都存在并且不同
			...
			TArrayView<TWrappedAttribute<FAnimStackAllocator>> ValuesArray = FStackAttributeContainerAccessor::GetValues(CustomAttributes, TypeIndex);
