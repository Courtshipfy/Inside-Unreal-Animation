...
namespace UE::Anim
{
	...
	void IAnimSequenceCompilingManager::FinishCompilation(TArrayView<UAnimSequence* const> InAnimSequences)
	{
		FAnimSequenceCompilingManager::Get().FinishCompilation(InAnimSequences); 
	}
	...
	FName FAnimSequenceCompilingManager::GetStaticAssetTypeName()
	{
	...
	void FAnimSequenceCompilingManager::OnPostReachabilityAnalysis()
	{
		if (GetNumRemainingAssets())
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(FAnimSequenceCompilingManager::CancelUnreachableMeshes);
