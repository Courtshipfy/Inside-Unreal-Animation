...
// [翻译失败: FAnimationPotentialTransition]
...
// [翻译失败: FAnimNode_StateMachine]

// Tries to get the instance information for the state machine
// [翻译失败: Tries to get the instance information for the state machine]
...
void FAnimNode_StateMachine::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
	...
	if (const FBakedAnimationStateMachine* Machine = GetMachineDescription())
	{
		...
		if (Machine->States.Num() > 0)
		{
			...
			// [翻译失败: Create a pose link for each state we can reach]
			...
			for (int32 StateIndex = 0; StateIndex < Machine->States.Num(); ++StateIndex)
			{
				...
				// [翻译失败: because conduits don't contain bound graphs, this link is no longer guaranteed to be valid]
				...
				// [翻译失败: also initialize transitions]
...
void FAnimNode_StateMachine::ConditionallyCacheBonesForState(int32 StateIndex, FAnimationBaseContext Context)
{
	...
	// [翻译失败: Check both the frame and counter]
	...
	// [翻译失败: of the one from the main anim instance. When the LOD of the main instance will change, we'll reset]
	// all the counters to match it which can cause them to run backwards
	// [翻译失败: all the counters to match it which can cause them to run backwards]
	if (!StateCacheBoneCounters[StateIndex].IsSynchronized_All(ProxyCachedBonesCounter))
	{
		...
		// [翻译失败: keep track of states that have had CacheBones called on.]
...
// [翻译失败: Temporarily turned off while we track down and fix https://jira.ol.epicgames.net/browse/OR-17066]
...
void FAnimNode_StateMachine::Update_AnyThread(const FAnimationUpdateContext& Context)
{
	...
	// [翻译失败: If we just became relevant and haven't been initialized yet, then reinitialize state machine.]
	...
	// [翻译失败: Remove expired transition requests]
#if WITH_EDITORONLY_DATA
	...
	if (Machine != nullptr)
	{
		...
		else if(!Machine->States.IsValidIndex(CurrentState))
		{
			...
			// [翻译失败: Attempting to catch a crash where the state machine has been freed.]
			// Reported as a symptom of a crash in UE-24732 for 4.10. This log message should not appear given changes to
			// [翻译失败: Reported as a symptom of a crash in UE-24732 for 4.10. This log message should not appear given changes to]
...
const FAnimNode_AssetPlayerRelevancyBase* FAnimNode_StateMachine::GetRelevantAssetPlayerInterfaceFromState(const FAnimInstanceProxy* InAnimInstanceProxy, const FBakedAnimationState& StateInfo) const
{
	...
	for (const int32& LinkedAnimNodeIdx : StateInfo.LayerNodeIndices)
	{
		...
		// [翻译失败: Try and retrieve the actual node object]
		if (const FAnimNode_LinkedAnimGraph* LinkedAnimGraph = InAnimInstanceProxy->GetNodeFromIndex<FAnimNode_LinkedAnimGraph>(LinkedAnimNodeIdx))
		{
			...
			// [翻译失败: Retrieve the AnimInstance running for this linked anim graph/layer]
...
bool FAnimNode_StateMachine::FindValidTransition(const FAnimationUpdateContext& Context, const FBakedAnimationState& StateInfo, /*out*/ FAnimationPotentialTransition& OutPotentialTransition, /*out*/ TArray<int32, TInlineAllocator<4>>& OutVisitedStateIndices)
{
	...
	// [翻译失败: so we can avoid doing unnecessary work (and infinite loops) by caching off states we have already checked]
	...
	// [翻译失败: Conduit 'states' have an additional entry rule which must be true to consider taking any transitions via the conduit]
	...
	for (int32 TransitionIndex = 0; TransitionIndex < NumTransitions; ++TransitionIndex)
	{
		...
		else if (TransitionRule.bAutomaticRemainingTimeRule)
		{
			...
			if (const FAnimNode_AssetPlayerRelevancyBase* RelevantPlayer = GetRelevantAssetPlayerInterfaceFromState(Context, StateInfo))
			{
				if (const UAnimationAsset* AnimAsset = RelevantPlayer->GetAnimAsset())
				{
					...
					if (RelevantPlayer->IsLooping() && AnimTimeRemaining > 0.f)
					{
						if (const FDeltaTimeRecord* DeltaTimeRecord = RelevantPlayer->GetDeltaTimeRecord())
						{
							if (DeltaTimeRecord->IsPreviousValid())
							{
								...
								// [翻译失败: deal with negative play rates]
					...
					// [翻译失败: For transitions that go to a conduit the user is not able to edit the transition's cross fade duration,]
					...
					// [翻译失败: therefore we force the cross fade duration to zero to ensure the transition is always triggerred upon reaching the end of the animation.]
					...
					// [翻译失败: Allow for used to determine the automatic transition trigger time or fallback to using cross fade duration.]
					...
					// [翻译失败: Trigger transition only if we have "CrossfadeTimeAdjustment" seconds left before reaching animation end boundary.]
		...
		{
			// Execute it and see if we can take this rule
			// [翻译失败: Execute it and see if we can take this rule]
		...
		if (ResultNode->bCanEnterTransition == TransitionRule.bDesiredTransitionReturnValue)
		{
			...
			// [翻译失败: if next state is a conduit we want to check for transitions using that state as the root]
			...
			// [翻译失败: otherwise we have found a content state, so we can record our potential transition]
			else
			{
				// clear out any potential transition we already have
				// [翻译失败: clear out any potential transition we already have]
				OutPotentialTransition.Clear();

				// fill out the potential transition information
				// [翻译失败: fill out the potential transition information]
...
void FAnimNode_StateMachine::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	if (ActiveTransitionArray.Num() > 0)
	{
		...
		//[翻译失败: each transition stomps over the last because they will already include the output from the transition before it]
		for (int32 Index = 0; Index < ActiveTransitionArray.Num(); ++Index)
		{
...
void FAnimNode_StateMachine::SetState(const FAnimationBaseContext& Context, int32 NewStateIndex, bool bAllowReEntry)
{
	...
	if (NewStateIndex != CurrentState || bAllowReEntry)
	{
		...
		// [翻译失败: This stops any zero length blends holding on to old weights]
		...
		// [翻译失败: Clear any currently cached blend weights for asset player nodes in layers.]
...
float FAnimNode_StateMachine::GetStateWeight(int32 StateIndex) const
{
	...
	if (NumTransitions > 0)
	{
		...
		// [翻译失败: Determine the overall weight of the state here.]
		...
		for (int32 Index = 0; Index < NumTransitions; ++Index)
		{
			...
			// [翻译失败: After the first transition, so source weight is the fraction of how much all previous transitions contribute to the final weight.]
			...
			// [翻译失败: So if our second transition is 50% complete, and our target state was 80% of the first transition, then that number will be multiplied by this weight]
			if (Index > 0)
			{
				TotalWeight *= SourceWeight;
			}
			//during the first transition the source weight represents the actual state weight
			//[翻译失败: during the first transition the source weight represents the actual state weight]
			...
			// [翻译失败: The next state weight is the alpha of this transition. We always just add the value, it will be reduced down if there are any newer transitions]
...
bool FAnimNode_StateMachine::QueryTransitionEvent(const int32 TransitionIndex, const FName& EventName) const
{
	...
	// [翻译失败: Assumes QueuedTransitionEvents is sorted by request creation time, i.e. index 0 is newest request, and that]
	// ContainsByPredicate returns the first (newest) request
	// [翻译失败: ContainsByPredicate returns the first (newest) request]
...
bool FAnimNode_StateMachine::QueryAndMarkTransitionEvent(const int32 TransitionIndex, const FName& EventName)
{
	...
	// [翻译失败: IndexOfByPredicate returns the first (newest) request]
...
const FPoseContext& FAnimNode_StateMachine::EvaluateState(int32 StateIndex, const FPoseContext& Context)
{
	...
	if (CachePosePtr == nullptr)
	{
		...
		{
			// Make sure CacheBones has been called before evaluating.
			// [翻译失败: Make sure CacheBones has been called before evaluating.]
			ConditionallyCacheBonesForState(StateIndex, Context);
