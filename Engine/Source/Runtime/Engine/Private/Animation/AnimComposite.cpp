...
void UAnimComposite::HandleAssetPlayerTickedInternal(FAnimAssetTickContext &Context, const float PreviousTime, const float MoveDelta, const FAnimTickRecord &Instance, struct FAnimNotifyQueue& NotifyQueue) const
{
	...
	ExtractRootMotionFromTrack(AnimationTrack, PreviousTime, PreviousTime + MoveDelta, FAnimExtractContext(), Context.RootMotionMovementParams);
}
...
FTransform UAnimComposite::ExtractRootMotion(const FAnimExtractContext& ExtractionContext) const
{
	return ExtractRootMotionFromRange(ExtractionContext.CurrentTime, static_cast<double>(ExtractionContext.DeltaTimeRecord.Delta), ExtractionContext);
}

FTransform UAnimComposite::ExtractRootMotionFromRange(double StartTime, double EndTime, const FAnimExtractContext& ExtractionContext) const
{
	FRootMotionMovementParams RootMotion;
	ExtractRootMotionFromTrack(AnimationTrack, StartTime, EndTime, ExtractionContext, RootMotion);
	return RootMotion.GetRootMotionTransform();
}

FTransform UAnimComposite::ExtractRootTrackTransform(const FAnimExtractContext& ExtractionContext, const FBoneContainer* RequiredBones) const
{
	if (const FAnimSegment* AnimSegment = AnimationTrack.GetSegmentAtTime(ExtractionContext.CurrentTime))
	{
		float SegmentTime = 0.0f;
		if (const UAnimSequenceBase* SequenceBase = AnimSegment->GetAnimationData(ExtractionContext.CurrentTime, SegmentTime))
		{
			FAnimExtractContext Context = ExtractionContext;
			Context.CurrentTime = static_cast<double>(SegmentTime);
			return SequenceBase->ExtractRootTrackTransform(Context, RequiredBones);
		}
	}
 ...
 // 返回最后一个有效值，以防我们请求动画复合结束时间之后的时间。
	if (!AnimationTrack.AnimSegments.IsEmpty())
	{
		...
		if (ExtractionContext.CurrentTime > AnimationTrack.AnimSegments[LastSegmentIndex].AnimEndTime)
		{
			const UAnimSequenceBase* SequenceBase = AnimationTrack.AnimSegments[LastSegmentIndex].GetAnimReference().Get();

			FAnimExtractContext Context = ExtractionContext;
			Context.CurrentTime = static_cast<double>(SequenceBase->GetPlayLength());
			return SequenceBase->ExtractRootTrackTransform(Context, RequiredBones);
		}
	}
