...
protected:
	...
	/** [翻译失败: Resets the compression buffer to defaults (no data)] */
	void Reset()
	{
		MaxError = 0.0f;
		SumError = 0.0;
	...
	void CompressTranslation_16_16_16(const FTranslationTrack& TranslationData, float ZeroingThreshold)
	{
		...
		for (int32 i = 0; i < NumKeys; ++i)
		{
			...
			if (bHasX)
			{
				X = FAnimationCompression_PerTrackUtils::CompressFixed16(V.X, AnimationCompressionUtils::LogScale);
				AppendBytes(&X, sizeof(X));
			}
			if (bHasY)
			{
				Y = FAnimationCompression_PerTrackUtils::CompressFixed16(V.Y, AnimationCompressionUtils::LogScale);
				AppendBytes(&Y, sizeof(Y));
			}
			if (bHasZ)
			{
				Z = FAnimationCompression_PerTrackUtils::CompressFixed16(V.Z, AnimationCompressionUtils::LogScale);
				AppendBytes(&Z, sizeof(Z));
			}

			const FVector3f DecompressedV(
				bHasX ? FAnimationCompression_PerTrackUtils::DecompressFixed16<AnimationCompressionUtils::LogScale>(X) : 0.0f,
				bHasY ? FAnimationCompression_PerTrackUtils::DecompressFixed16<AnimationCompressionUtils::LogScale>(Y) : 0.0f,
				bHasZ ? FAnimationCompression_PerTrackUtils::DecompressFixed16<AnimationCompressionUtils::LogScale>(Z) : 0.0f);
...
/** [翻译失败: Resamples a track of Scale keys] */
