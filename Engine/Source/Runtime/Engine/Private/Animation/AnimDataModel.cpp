...
void UAnimDataModel::GenerateStateHash(HasherType& Hasher, const FGuidGenerationSettings& InSettings) const
{
	...
	{
		Hasher.BeginObject(TEXT("BAT"));
		for (const FBoneAnimationTrack& Track : BoneAnimationTracks)
		{
			Hasher.BeginObject();
			Hasher.UpdateArray(Track.InternalTrackData.PosKeys, TEXT("P"));
			Hasher.UpdateArray(Track.InternalTrackData.RotKeys, TEXT("R"));
			Hasher.UpdateArray(Track.InternalTrackData.ScaleKeys, TEXT("S"));
			Hasher.EndObject();
		}
		Hasher.EndObject();
	}
	...
	{
		Hasher.BeginObject(TEXT("FC"));
		for (const FFloatCurve& Curve : CurveData.FloatCurves)
		{
			Hasher.BeginObject();
			Hasher.UpdateLegacyString(Curve.GetName().ToString(), TEXT("N"));
			Hasher.UpdateRichCurve(Curve.FloatCurve, TEXT("C"));
			Hasher.EndObject();
		}
		Hasher.EndObject();

		Hasher.UpdateTransformCurves(CurveData.TransformCurves, TEXT("TC"));
	}
...
FGuid UAnimDataModel::GenerateGuid(const FGuidGenerationSettings& InSettings) const
{
...
#if WITH_EDITOR
FString UAnimDataModel::GenerateDebugStateString() const
{
