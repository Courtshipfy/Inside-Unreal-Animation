	...
	, bUseCompressedData(false)
{
	if (InSequenceBase->HasCurveData(InName, false))
	{
		if (const UAnimSequence* InSequence = Cast<UAnimSequence>(InSequenceBase))
		{
			...
			{
				UAnimSequence::FScopedCompressedAnimSequence CompressedAnimSequence = InSequence->GetCompressedData();
				if (UAnimCurveCompressionCodec_UniformIndexable* IndexableCompressedCurve = Cast<UAnimCurveCompressionCodec_UniformIndexable>(CompressedAnimSequence.Get().CurveCompressionCodec))
				{
					bUseCompressedData = IndexableCompressedCurve->GetCurveBufferAndSamples(CompressedAnimSequence.Get(), InName, CompressedBuffer, NumSamples, SampleRate);
				}
				else
				{
...
int64 UAnimCurveCompressionCodec_UniformIndexable::EstimateCompressionMemoryUsage(const UAnimSequence& AnimSequence) const
{
