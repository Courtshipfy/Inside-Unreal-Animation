...
class UAnimBlueprint : public UBlueprint, public IInterface_PreviewMeshProvider
{
	...
	// [翻译失败: Inspects the hierarchy and looks for an override for the requested node GUID]
	// @param NodeGuid - Guid of the node to search for
	// [翻译失败: @param NodeGuid - Guid of the node to search for]
...
public:
	...
	// [翻译失败: particular point of the anim graph and control debug draw for unselected anim nodes).]
	UPROPERTY()
