...
namespace UE::Anim
{
 ...
 // 已弃用，使用 FINertializationRequest 代替
...
/** 跟踪图中活动槽节点的状态 */
	...
	/** 正在播放的蒙太奇的本地权重（插槽节点本地） */
	/** 这用于提取动画。如果 Mesh 存在，则会被 Mesh->Skeleton 覆盖 */
	...
	/** 该槽节点的全局权重 */
{
	...
	FName BindingName;
#endif
	/** 用于检查游戏线程的标记，表明我们需要在评估后调用中运行 PostUpdateAnimation() */
...
/** Tracks state of active slot nodes in the graph */
	/** 如果为 true，则链接实例将使用主实例的蒙太奇数据。 （即在主实例上播放蒙太奇也会在链接层上播放。） */
/** 跟踪图中活动槽节点的状态 */
...
class UAnimInstance : public UObject
{
 ...
 // 设置此蓝图从何处提取根运动
	...
	/** 如果此 AnimInstance 具有使用“CopyPoseFromMesh”的节点，则为 true。 */
	UPROPERTY(meta = (BlueprintCompilerGeneratedDefaults))
	/** 将空姿势快照添加到内部快照缓存（或回收现有姿势快照，如果该名称已在使用中） */
...
public:
 ...
 // 获取我们是否已初始化动画实例。如果 UninitializeAnimation() 被调用，这将返回 false。
	...
	/** 与 Montage_Stop 相同，但提供了所有混合设置，而不是使用蒙太奇资源上的设置*/
...
public:
 ...
 // 返回创建此 AnimInstance 的骨架网格物体组件
	/** 播放动画蒙太奇。返回动画蒙太奇的长度（以秒为单位）。如果播放失败则返回0.f。 */
	UFUNCTION(BlueprintCallable, Category = "Animation")
	/** 更改动画蒙太奇播放速率。 NewPlayRate = 1.0 是默认播放速率。 */
	...
	/** 播放动画蒙太奇。与 Montage_Play 相同，但您可以为 Blend In 设置指定 AlphaBlend。 */
	/** 如果动画蒙太奇处于活动状态，则返回 true。如果 Montage 引用为 NULL，则当任何 Montage 处于活动状态时，它将返回 true。 */
	/** 获取“主”动画实例，即托管在骨架网格物体组件上的动画实例 */
...
public:
	...
	/** 如果 Montage 当前未激活，则返回 true。 （无效或混合） */
	/** 与 Montage_Stop 相同，但提供了所有混合设置，而不是使用蒙太奇资源上的设置*/
	/** 在动画评估后执行 */
	UFUNCTION(BlueprintImplementableEvent)
	...
	/** 在工作线程上更新动画蓝图时执行，就在图形更新之前 */
	/** 使蒙太奇跳转到指定部分。如果蒙太奇引用为 NULL，它将对所有活动蒙太奇执行此操作。 */
	/** 如果当前有一个动画蒙太奇处于活动状态并且正在播放，该动画蒙太奇是根据提供的动画创建的，则返回 true。 */
...
public:
 ...
 // 获取子系统的实例驻留数据。如果该类型的子系统不存在，则返回 nullptr。
	/** 获取指定状态机的运行时实例 */
 ...
 // 获取子系统的实例驻留数据。如果该类型的子系统不存在，则会断言。
	...
	/** 返回指定（命名）动画图中 FAnimNode_AssetPlayerBase 类的所有动画节点 */
	{
		FAnimSubsystemInstance* Subsystem = FindSubsystem(SubsystemType::StaticStruct());
	/** 获取资产播放器节点当前累积时间的分数 */
	/** 返回指定（命名）动画图中实现 FAnimNode_AssetPlayerRelevancyBase 的所有动画节点 */
		check(Subsystem);
		return static_cast<SubsystemType&>(*Subsystem);
	}
...
public:
	...
	/** 获取源状态中最相关动画的剩余时间（作为持续时间的一部分） */
	/** 从 GameThread 上的 AnimInstanceProxy 访问更新计数器的只读版本。 */
	UFUNCTION(BlueprintPure, Category = "Animation|State Machines", meta = (BlueprintInternalUseOnly = "true", AnimGetter = "true", GetterContext = "Transition", BlueprintThreadSafe))
	/** 尝试将转换请求排队，如果请求成功则返回 true */
	...
	/** 触发 AnimNotify。  请注意，此版本不提供该事件的任何上下文 **/
	/** 返回指定（命名）动画图中 FAnimNode_AssetPlayerRelevancyBase 类的所有动画节点 */
...
public:
	...
	*/
	...
	/** 获取当前 RootMotion FAnimMontageInstance（如果有）。否则为 NULL。 */
	/** This returns last up-to-date list of active curve names */
	/** 重置运行模拟式更新的任何动态（例如传送、时间跳跃等） */
	/** 这将返回最新的活动曲线名称列表 */
	/** 获取当前累积的根运动，将其从过程中的 AnimInstance 中删除 */
	UFUNCTION(BlueprintPure, Category = "Animation|Curves", meta=(BlueprintThreadSafe))
...
public:
 ...
 // 显示有关 AnimInstance 的调试信息。可以重写以添加子类的自定义信息。
