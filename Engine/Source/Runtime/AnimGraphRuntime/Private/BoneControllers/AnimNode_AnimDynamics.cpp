...
TAutoConsoleVariable<int32> CVarEnableScaling(TEXT("p.AnimDynamics.ScaleSupport"), 1, TEXT("Enables/Disables scaling support for anim dynamics nodes."));
...
FAutoConsoleVariableRef CVarAnimDynamicsSpaceAngularAlpha(TEXT("p.AnimDynamics.Space.SimSpaceAngularAlpha"), AnimDynamics_SimSpaceOverride.SimSpaceAngularAlpha, TEXT("AnimDynamics SimSpaceSettings overrides"), ECVF_Default);
FAutoConsoleVariableRef CVarAnimDynamicsSpaceMaxCompAngVel(TEXT("p.AnimDynamics.Space.MaxAngularVelocity"), AnimDynamics_SimSpaceOverride.MaxAngularVelocity, TEXT("AnimDynamics SimSpaceSettings overrides"), ECVF_Default);
...
void FAnimNode_AnimDynamics::DrawBodies(FComponentSpacePoseContext& InContext, const TArray<FAnimPhysRigidBody*>& InBodies)
{
	...
	for(int32 BodyIndex = 0 ; BodyIndex < NumBodies ; ++BodyIndex)
	{
		...
		FTransform Transform(Body.Pose.Orientation, Body.Pose.Position, FVector(Body.Pose.Scale));
...
FAnimPhysSimSpaceSettings::FAnimPhysSimSpaceSettings()
	: SimSpaceAngularAlpha(0)
...
#endif
{
	...
	PreviousSimSpaceAngularVelocity = FVector::ZeroVector;
...
void FAnimNode_AnimDynamics::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
	...
	PreviousSimSpaceTM = PreviousCompWorldSpaceTM;
...
FTransform FAnimNode_AnimDynamics::GetSimToWorldSpaceTransform(AnimPhysSimSpaceType SimSpace, FComponentSpacePoseContext& Output) const
{
	FTransform OutTransform;

	if (CVarDisableWorldSpace.GetValueOnAnyThread() && SimSpace == AnimPhysSimSpaceType::World)
	{
		SimSpace = AnimPhysSimSpaceType::Component;
	}
	...
	case AnimPhysSimSpaceType::Component:
	{
		OutTransform = Output.AnimInstanceProxy->GetComponentTransform();
	...
	case AnimPhysSimSpaceType::Actor:
	{
		OutTransform = Output.AnimInstanceProxy->GetActorTransform();
	...
	case AnimPhysSimSpaceType::RootRelative:
	{
		...
		OutTransform = RelativeBoneTransform;
...
void FAnimNode_AnimDynamics::CalculateSimulationSpaceMotion(
	AnimPhysSimSpaceType        Space,
	FAnimPhysSimSpaceSettings	Settings,
	...
	FVector& SpaceAngularAcc)
{
	...
	PreviousSimSpaceAngularVelocity = SpaceAngularVel;
...
void FAnimNode_AnimDynamics::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	{
		const int bScalingEnabled = CVarEnableScaling.GetValueOnAnyThread();
		...
		if (bScalingEnabled)
		{
			const FTransform SimWorldTransform = GetSimToWorldSpaceTransform(SimulationSpace, Output);
			...
			if(SimWorldTransform.GetScale3D() != PreviousSimSpaceTM.GetScale3D())
			{
				...
				for (int32 ActiveIndex : ActiveBoneIndices)
				{
					...
					float Scale = BoundBoneTransform.GetScale3D().X;
					if (RigidBody.Pose.Scale != Scale)
					{
						FAnimPhys::ScaleRigidBodyMass(&RigidBody, Scale/RigidBody.Pose.Scale);

						TArray<FAnimPhysShape> BodyShapes;

						BodyShapes.Add(FAnimPhysShape::MakeBox(PhysicsBodyDef.BoxExtents * Scale));
						RigidBody.Pose.Scale = Scale;

						switch (RigidBody.CollisionType)
						{
						case AnimPhysCollisionType::CustomSphere:
							RigidBody.SphereCollisionRadius = static_cast<float>(PhysicsBodyDef.SphereCollisionRadius * Scale);
							break;
						case AnimPhysCollisionType::InnerSphere:
							RigidBody.SphereCollisionRadius = static_cast<float>(PhysicsBodyDef.BoxExtents.GetAbsMin() * Scale / 2.0);
							break;
						case AnimPhysCollisionType::OuterSphere:
							RigidBody.SphereCollisionRadius = static_cast<float>(PhysicsBodyDef.BoxExtents.GetAbsMax() * Scale / 2.0);
							break;
						default:
							break;
						}
						RigidBody.Shapes = BodyShapes;
					}
				}
			}
		}
		...
		while(BodiesToReset.Num() > 0)
		{
			...
			if(BodyToReset && BodyToReset->RigidBody.BoundBone.IsValidToEvaluate(RequiredBones))
			{
				...
				PhysBody.Pose.Scale = BoneTransform.GetScale3D().X;
		...
		if (ShouldDoPhysicsUpdate() && NextTimeStep > AnimPhysicsMinDeltaTime)
		{
			...
			FVector SimFrameAngularVelocity;
			FVector SimFrameAngularAcc;

			FAnimPhysSimSpaceSettings UseSimSpaceSettings = SimSpaceSettings;
			if (bAnimDynamics_SimSpace_EnableOverride)
			{
				if (AnimDynamics_SimSpaceOverride.SimSpaceAngularAlpha >= 0.0f)
				{
					UseSimSpaceSettings.SimSpaceAngularAlpha = AnimDynamics_SimSpaceOverride.SimSpaceAngularAlpha;
				}
				if (AnimDynamics_SimSpaceOverride.MaxAngularVelocity >= 0.0f)
				{
					UseSimSpaceSettings.MaxAngularVelocity = AnimDynamics_SimSpaceOverride.MaxAngularVelocity;
				}
				if (AnimDynamics_SimSpaceOverride.MaxAngularAcceleration >= 0.0f)
				{ 
					UseSimSpaceSettings.MaxAngularAcceleration = AnimDynamics_SimSpaceOverride.MaxAngularAcceleration;
				}
			}

			CalculateSimulationSpaceMotion(SimulationSpace, SimSpaceSettings, Output, NextTimeStep, SimFrameAngularVelocity, SimFrameAngularAcc);
			...
			if (CVarEnableAdaptiveSubstep.GetValueOnAnyThread() == 1)
			{
				...
				{
					UpdateLimits(Output);
					FAnimPhys::PhysicsUpdate(FixedTimeStep, SimBodies, LinearLimits, AngularLimits, Springs, SimSpaceGravityDirection, OrientedExternalForce, ComponentLinearAcc, SimFrameAngularAcc, SimFrameAngularVelocity, NumSolverIterationsPreUpdate, NumSolverIterationsPostUpdate);
				}
			}
			else
			{
				...
				UpdateLimits(Output);
				FAnimPhys::PhysicsUpdate(NextTimeStep, SimBodies, LinearLimits, AngularLimits, Springs, SimSpaceGravityDirection, OrientedExternalForce, ComponentLinearAcc, SimFrameAngularAcc, SimFrameAngularVelocity, NumSolverIterationsPreUpdate, NumSolverIterationsPostUpdate);
			}
		...
		if (bDoEval)
		{
			...
			for (int32 Idx = 0; Idx < Bodies.Num(); ++Idx)
			{
				...
				if (bScalingEnabled)
				{
					NewBoneTransform = FTransform(CurrentBody.Pose.Orientation, CurrentBody.Pose.Position - CurrentBody.Pose.Orientation.RotateVector(PhysicsBodyJointOffsets[Idx]), FVector(CurrentBody.Pose.Scale));
				}
				else
				{
					NewBoneTransform = FTransform(CurrentBody.Pose.Orientation, CurrentBody.Pose.Position - CurrentBody.Pose.Orientation.RotateVector(PhysicsBodyJointOffsets[Idx]));
				}
...
{
	const int bScalingSupportEnabled = CVarEnableScaling.GetValueOnAnyThread();
	...
	case ETeleportType::ResetPhysics:
	{
		...
		for (FAnimPhysBodyDefinition& PhysicsBodyDef : PhysicsBodyDefinitions)
		{
			...
			if (bScalingSupportEnabled)
			{
				BodyShapes.Add(FAnimPhysShape::MakeBox(PhysicsBodyDef.BoxExtents * BodyTransform.GetScale3D()));
				BodyTransform.SetTranslation(BodyTransform.TransformPosition(PhysicsBodyDef.LocalJointOffset));
			}
			else
			{
			...
			PhysicsBody.Pose.Scale = BodyTransform.GetScale3D().X;
			...
			{
			case AnimPhysCollisionType::CustomSphere:
				PhysicsBody.SphereCollisionRadius = static_cast<float>(bScalingSupportEnabled ? PhysicsBodyDef.SphereCollisionRadius * BodyTransform.GetScale3D().X : PhysicsBodyDef.SphereCollisionRadius);
				break;
			case AnimPhysCollisionType::InnerSphere:
				PhysicsBody.SphereCollisionRadius = static_cast<float>(bScalingSupportEnabled ? (PhysicsBodyDef.BoxExtents.GetAbsMin() * BodyTransform.GetScale3D().X / 2.0) : PhysicsBodyDef.BoxExtents.GetAbsMin() / 2.0);
				break;
			case AnimPhysCollisionType::OuterSphere:
				PhysicsBody.SphereCollisionRadius = static_cast<float>(bScalingSupportEnabled ? (PhysicsBodyDef.BoxExtents.GetAbsMax() * BodyTransform.GetScale3D().X / 2.0) : PhysicsBodyDef.BoxExtents.GetAbsMax() / 2.0);
				break;
			default:
				break;
			}
	...
	PreviousCompWorldSpaceTM = Output.AnimInstanceProxy->GetComponentTransform();
	PreviousActorWorldSpaceTM = Output.AnimInstanceProxy->GetActorTransform();
	PreviousSimSpaceTM = GetSimToWorldSpaceTransform(SimulationSpace, Output);
}
...
FTransform FAnimNode_AnimDynamics::GetComponentSpaceTransformFromSimSpace(AnimPhysSimSpaceType SimSpace, FComponentSpacePoseContext& Output, const FTransform& InSimTransform, const FTransform& InCompWorldSpaceTM, const FTransform& InActorWorldSpaceTM) const
{
	...
	if (SimSpace == AnimPhysSimSpaceType::World && CVarDisableWorldSpace.GetValueOnAnyThread())
	{
		SimSpace = AnimPhysSimSpaceType::Component;
	}
...
FVector FAnimNode_AnimDynamics::TransformWorldVectorToSimSpaceScaled(FComponentSpacePoseContext& Output, const FVector& InVec) const
{
	FVector OutVec = InVec;
	...
	case AnimPhysSimSpaceType::Actor:
	{
		OutVec = Output.AnimInstanceProxy->GetActorTransform().InverseTransformVector(OutVec);
	...
	case AnimPhysSimSpaceType::RootRelative:
	{
		...
		FTransform RelativeBoneTransform = Output.Pose.GetComponentSpaceTransform(RootBoneCompactIndex);
		RelativeBoneTransform = Output.AnimInstanceProxy->GetComponentTransform() * RelativeBoneTransform;
		OutVec = RelativeBoneTransform.InverseTransformVector(OutVec);
